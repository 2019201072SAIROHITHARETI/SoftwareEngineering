...
def reg_user(uname,pwd):
    ...
    flag = 0
    data_base = sql.connect(host='localhost',user='root',passwd='',database='LOGIN_APP')
    c = data_base.cursor()
    c.execute('select name from LOGIN_DETAILS')
    l = c.fetchall()
    for i in l:
        if uname == i[0]:
          flag=1
    ...
    if (flag==1):
       ...
       c.execute(f'insert into LOGIN_DETAILS values("{uname}","{pwd}")')
       data_base.commit()
